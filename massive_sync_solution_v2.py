...
def get_all_bucket_files_direct():
    """Get all bucket files using direct table queries."""
    print("📋 Getting all bucket files via direct queries...")
    all_files = []
    offset = 0
    ...
    while True:
        try:
            ...
            chunk_files = [row['name'] for row in result.data if row['name'].lower().endswith('.jpg')]
            all_files.extend(chunk_files)
            
            print(f"   Retrieved chunk {offset//limit + 1}: {len(chunk_files)} JPG files (total: {len(all_files)})")
...
def analyze_massive_bucket():
    ...
    bucket_files = get_all_bucket_files_direct()
